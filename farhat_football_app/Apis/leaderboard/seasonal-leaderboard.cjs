...
ORDER BY total_goals DESC, total_assists DESC, man_of_the_match_count DESC;
		...
		//   COUNT(*) AS matches_played,
		//   COUNT(CASE WHEN wt.winning_team_id IS NOT NULL AND mp.team_id = wt.winning_team_id THEN 1 END) AS wins,
		//   COUNT(CASE WHEN m.man_of_the_match = p.player_id THEN 1 END) AS man_of_the_match_count
		// FROM match_players mp
		// JOIN players p ON mp.player_id = p.player_id
		// JOIN matches m ON mp.match_id = m.match_id
		// LEFT JOIN winning_teams wt ON mp.match_id = wt.match_id
		// WHERE EXTRACT(YEAR FROM m.match_date) = $1
		//   AND EXTRACT(MONTH FROM m.match_date) BETWEEN $2 AND $3
